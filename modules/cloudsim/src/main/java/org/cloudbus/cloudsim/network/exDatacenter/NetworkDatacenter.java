...
public class NetworkDatacenter extends Datacenter {
	...
	public void shutdownEntity() {
		...
		//DCMngUtility.resultFile.println("Simulation ended at CloudSim clock "+CloudSim.clock());
	...
	public Map<Integer, Switch> getRootSwitch() {
		Map<Integer, Switch> rootswitch = new HashMap<Integer, Switch>();
		for (Entry<Integer, Switch> es : Switchlist.entrySet()) {
			if (es.getValue().level == NetworkConstants.ROOT_LEVEL) {
				rootswitch.put(es.getKey(), es.getValue());
			}
		}
		return rootswitch;
	...
	public Map<Integer, Switch> getAggSwitch() {
		Map<Integer, Switch> aggswitch = new HashMap<Integer, Switch>();
		for (Entry<Integer, Switch> es : Switchlist.entrySet()) {
			if (es.getValue().level == NetworkConstants.Agg_LEVEL) {
				aggswitch.put(es.getKey(), es.getValue());
			}
		}
		return aggswitch;
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			System.out.println("!!!!!!!ERROR :"+ err_hostid + ","+ err_vmid +","+ err_usrid);
			e.printStackTrace();
		}catch (Exception e) {
