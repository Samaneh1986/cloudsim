...
public class Switch extends SimEntity {
	...
	public double switching_delay;
		/**
		 * The total resources which are available via physical hosts
		 * which are connected directly or indirectly to this switch.
		 */
	public int has_EG_value = 0;
	...
	public Switch(String name, int level, NetworkDatacenter dc) {
		super(name);
		//uplinkSwFullPrc = new ArrayList<Double>();
		//downlinkSwFullPrc = new ArrayList<Double>();
		this.level = level;
		this.dc = dc;
	}
	...
	protected void processpacket_down(SimEvent ev) {
		...
		if (level == NetworkConstants.EDGE_LEVEL) {
			//System.out.println("proccess down for edge sw in switch class!!");
